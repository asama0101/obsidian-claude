...
2. プロジェクト解決フロー: `python .claude/skills/vault/scripts/list_projects.py`
   を実行し、標準出力のJSON（`{"projects": [...]}`）から`10_Projects/`直下の
   候補一覧を取得する。次の3択をユーザーに確認する（**自動推測はしない**）。
   - 既存プロジェクトから選ぶ
   - 新規プロジェクトを作成する（`project-add`スキルを呼び出す）
   - プロジェクトなしで進める
3. `python .claude/skills/vault/scripts/task_save.py --title <text> [--project <project>] [--due-date YYYY-MM-DD]`
