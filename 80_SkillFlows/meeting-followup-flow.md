...
- プロジェクトなしで進める場合、保存先は`20_Areas/Tasks/`に固定される。
- `task_save.py`の`--project`は任意（省略時`None`）であり、省略時は`20_Areas/Tasks/`に保存される。「プロジェクトなし」は正式な選択肢として扱われる。
...
- 保存先はプロジェクトありの場合`10_Projects/<名前>/Tasks/`、プロジェクトなしの場合`20_Areas/Tasks/`になる。
